...
var path = host+'/mkt/asset2cash' //如果url路径有前缀请加上;
...
(function () {
    ...
    w.Hero = {
                    ...
                    success: function(data){
                        ...
                        if (data.result === 'success') {
                            ...
                            if (localStorage.fastLoginType === 'kFaskLoginGesture') {
                                Hero.out({command:'present:'+path+'/protect_gesture.html'});
                            } else if (localStorage.fastLoginType === 'kFaskLoginTouchID') {
                                Hero.out({command:'present:'+path+'/protect_touchID.html'});
                            } else {
                                Hero.out({command:'present:'+path+'/login.html'});
                            }
                                        ...
                                        if (window.errors) {
                                            for (var i = window.errors.length - 1; i >= 0; i--) {
                                                ...
                                                if (error.code == data.code) {
                                                    cmsError = true;
                                                    Hero.out({datas:{name:'toast',text:error['zh-CN']}});
                                                };
                                            };
                                        };
                                        if (!cmsError) {
                                            Hero.out({datas:{name:'toast',text:'网络返回出错，错误代码:'+data.code}});
                                        };
                                    }
                                }
                            }
        ...
        createElementFromJson:function(json){
            ...
            if (viewName ) {
                var className = Hero.camelCase2bar(viewName);
